...
class WorkerLoggingTests(unittest.TestCase):
    ...
    def test_preview_worker_uses_task_subprocess(self) -> None:
        backend_root = Path(__file__).resolve().parents[1]
        source = (backend_root / "app" / "worker.py").read_text(encoding="utf-8")

        self.assertIn("run_task_in_subprocess(task_id, worker_id, redis_client, run_preview_task", source)
        self.assertNotIn("run_preview_task_in_process(task_id, worker_id, redis_client)", source)
