...
class StorageResponseTests(unittest.TestCase):
    ...
    def test_pipeline_parameter_schema_lists_supported_pipelines_and_fields(self) -> None:
        with TestClient(app) as client:
            ...
            self.assertEqual(pipelines[FINE_PIPELINE_NAME]["defaults"]["indoor"]["blur_code_dim"], 8)
            self.assertEqual(pipelines[FINE_PIPELINE_NAME]["defaults"]["indoor"]["fine_image_max_side"], 0)
