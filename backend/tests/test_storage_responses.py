...
class StorageResponseTests(unittest.TestCase):
    ...
    def test_pipeline_parameter_schema_lists_supported_pipelines_and_fields(self) -> None:
        with TestClient(app) as client:
            ...
            self.assertEqual(pipelines[FINE_PIPELINE_NAME]["defaults"]["indoor"]["max_n_gaussian"], -1)
            ...
            self.assertFalse(pipelines[FINE_PIPELINE_NAME]["defaults"]["indoor"]["densify_with_depth"])
            self.assertTrue(pipelines[FINE_PIPELINE_NAME]["defaults"]["indoor"]["dash_enable"])
            self.assertEqual(pipelines[FINE_PIPELINE_NAME]["defaults"]["indoor"]["resolution_mode"], "freq")
            self.assertEqual(pipelines[FINE_PIPELINE_NAME]["defaults"]["indoor"]["densify_mode"], "freq")
            self.assertFalse(pipelines[FINE_PIPELINE_NAME]["defaults"]["indoor"]["Grouping"])
    ...
    def test_fine_task_can_start_without_preview_after_eight_images(self) -> None:
        with TestClient(app) as client, patch("app.main.enqueue_fine_task", return_value=None):
            ...
            self.assertEqual(payload["options"]["fine_sfm_backend"], "pycolmap")
