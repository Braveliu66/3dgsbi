...
def _parse_kv_debug_interval(val: str) -> int:
    ...
    Accepts: "" (disabled) | "0" (disabled) | "1" (every frame)
             | "N" (every N frames) | any other truthy string 鈫?every frame.
    """
    if not val:
        return 0
    try:
...
def _log_kv_stats(model, label: str = "") -> None:
    ...
    reported 鈥?it is unused when 3D RoPE is disabled but helps catch mismatches.
    """
    try:
        parts = []
    ...
    except Exception as e:  # pragma: no cover 鈥?debug helper, never fatal
...
class GCTStream(GCTBase):
    ...
    # 鈹€鈹€ Flow-based keyframe helpers 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
    ...
    def _stitch_windows(
        ...
        For each temporal key the method builds a slice table first 鈥?every
    ...
    def _pairwise_alignment(
        ...
        # Scale from depth 鈥?aggregate across all paired keyframes in overlap.
        s_ab = unit_s.clone()
