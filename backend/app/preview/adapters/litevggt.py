...
def run(ctx: PreviewContext) -> PreviewResult:
    ...
    keep_ratio = _read_float(ctx.options.get("litevggt_keep_ratio"), 0.42)
    ...
    preprocess_mode = str(ctx.options.get("litevggt_preprocess_mode") or "crop")
    point_selection_strategy = str(ctx.options.get("litevggt_point_selection_strategy") or "global_confidence")
