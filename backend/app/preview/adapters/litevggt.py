...
from app.litevggt_defaults import (
    LITEVGGT_DEFAULT_PREVIEW_SCENE_PROFILE,
    LITEVGGT_OFFICIAL_PAD_PRESET,
    LITEVGGT_OFFICIAL_RUNTIME_DEFAULTS,
    LITEVGGT_SCENE_PROFILE_DEFAULTS as SHARED_LITEVGGT_SCENE_PROFILE_DEFAULTS,
)
...
DEFAULT_PREVIEW_SCENE_PROFILE = LITEVGGT_DEFAULT_PREVIEW_SCENE_PROFILE
LITEVGGT_SCENE_PROFILE_DEFAULTS = {
    profile: dict(defaults)
    for profile, defaults in SHARED_LITEVGGT_SCENE_PROFILE_DEFAULTS.items()
}
...
def run(ctx: PreviewContext) -> PreviewResult:
    ...
    parameter_sources = _litevggt_parameter_sources(ctx.options)
    official_mismatches = _official_default_mismatches(
        {
            "litevggt_keep_ratio": keep_ratio,
            "preview_max_points": max_points,
            "litevggt_max_input_frames": max_input_frames,
            "litevggt_target_size": target_size,
            "litevggt_frame_stride": frame_stride,
            "litevggt_depth_conf_thresh": depth_conf_thresh,
            "litevggt_preprocess_mode": preprocess_mode,
            "litevggt_inference_mode": inference_mode,
            "litevggt_chunk_size": chunk_size,
            "litevggt_overlap": overlap,
            "litevggt_loop_closure": loop_closure,
            "litevggt_keyframe_target": keyframe_target,
            "litevggt_min_frame_gap": min_frame_gap,
            "litevggt_min_scene_change": min_scene_change,
            "litevggt_window_voxel_diag_ratio": window_voxel_diag_ratio,
            "litevggt_final_voxel_diag_ratio": final_voxel_diag_ratio,
            "litevggt_point_selection_strategy": point_selection_strategy,
            "litevggt_axis_trim_low_quantile": axis_trim_low_quantile,
            "litevggt_axis_trim_high_quantile": axis_trim_high_quantile,
            "litevggt_spatial_keep_quantile": spatial_keep_quantile,
        }
    )
    params: dict[str, Any] = {
        "preview_scene_profile": scene_profile,
        "official_pad_preset": LITEVGGT_OFFICIAL_PAD_PRESET,
        "official_pad_mismatches": official_mismatches,
        "param_sources": parameter_sources,
    ...
    metrics = run_litevggt_pointcloud(
        ...
        keep_ratio=None if _matches_official("litevggt_keep_ratio", keep_ratio) else keep_ratio,
        max_points=max_points,
        max_input_frames=max_input_frames,
        target_size=None if _matches_official("litevggt_target_size", target_size) else target_size,
    ...
    point_radius_scale = _read_bounded_float(
        ...
        profile_defaults.get("preview_fixed_splat_radius_scale", 0.22),
        0.05,
        20.0,
    )
    fixed_splat_opacity = _read_bounded_float(
        ctx.options.get("preview_fixed_splat_opacity"),
        profile_defaults.get("preview_fixed_splat_opacity", 0.55),
        0.05,
        0.99,
    )
...
def _litevggt_parameter_sources(options: dict[str, Any]) -> dict[str, str]:
    sources = options.get("litevggt_parameter_sources")
    return dict(sources) if isinstance(sources, dict) else {}


def _matches_official(key: str, value: Any) -> bool:
    official = LITEVGGT_OFFICIAL_RUNTIME_DEFAULTS.get(key)
    if isinstance(official, float):
        try:
            return abs(float(value) - official) < 1e-9
        except (TypeError, ValueError):
            return False
    return value == official


def _official_default_mismatches(values: dict[str, Any]) -> dict[str, dict[str, Any]]:
    mismatches: dict[str, dict[str, Any]] = {}
    for key, value in values.items():
        if key not in LITEVGGT_OFFICIAL_RUNTIME_DEFAULTS or _matches_official(key, value):
            continue
        mismatches[key] = {"actual": value, "official": LITEVGGT_OFFICIAL_RUNTIME_DEFAULTS[key]}
    return mismatches
