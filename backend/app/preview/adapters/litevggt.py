...
def run(ctx: PreviewContext) -> PreviewResult:
    ...
    keep_ratio = float(keep_ratio_value) if keep_ratio_value is not None else 0.70
    max_points = int(ctx.options.get("preview_max_points") or 5050_000)
    ...
    target_size = int(target_size_value) if target_size_value else 518
    frame_stride = _read_optional_int(ctx.options.get("litevggt_frame_stride"))
    depth_conf_thresh = _read_optional_float(ctx.options.get("litevggt_depth_conf_thresh"), 2.5)
