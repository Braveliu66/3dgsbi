...
COLMAP_DEFAULTS_PRESET = "dash_deblur_group_defaults_2026_05_18_v2"
...
def colmap_fields() -> list[dict[str, Any]]:
    return [
        ...
        field("fine_colmap_max_num_matches", "Max matches", "number", "COLMAP", "Maximum COLMAP feature matches per pair.", min=1024, max=65536, step=1),
        field("fine_colmap_sequential_overlap", "Sequential overlap", "number", "COLMAP", "Sequential matcher overlap for ordered captures.", min=4, max=200, step=1),
        ...
        field("fine_deblur_mode", "Deblur mode", "select", "Deblur", "Deblur physics branch used during training. Mix keeps the mixed blur preset instead of auto-selecting a single branch.", options=["mix", "motion", "defocus", "sharp"]),
        ...
        field("position_lr_final", "Final position LR", "number", "Deblur", "Final Gaussian position learning rate.", min=0, max=1, step=0.0000001),
        field("percent_dense", "Dense percent", "number", "Densify", "Scene extent threshold for clone/split densification.", min=0, max=1, step=0.001),
        field("lambda_dssim", "DSSIM weight", "number", "Deblur", "DSSIM loss weight during training.", min=0, max=1, step=0.01),
