...
class SfMemory():
    ...
    def initialize(self, cluster_pred_all, best_kf_idx):
        ...
        if 'intrinsic' in predictions:
            self.intrinsics[predictions['idx']] = predictions['intrinsic']
    ...
    def update(self, res, align=False, num_kf=None, add_kf=True, adaptive=False):
        ...
        if len(map_idx_update) > 0:
            ...
            if intrinsic_local is not None:
                self.intrinsics[map_idx_update] = intrinsic_local[mask] if adaptive and 'mask' in locals() else intrinsic_local
