...
- RoMA/EDGS 权重支持 `MODEL_AUTO_DOWNLOAD=true` 时自动下载；也可以提前手动放入共享 `model-cache`。
...
- When `fine_edgs_enabled=false`, pycolmap sparse initialization and sparse point compensation remain available.
