...
- Deblurring-3DGS GTnet, DashGaussian scheduling, and non-destructive Group Training live under `worker/trainer/dash_deblur_group_gs`; backend code preserves `mix` as a mixed deblur preset, generates scene-specific mixed/motion/defocus presets, runs `train.py`, validates `final.ply`, and converts `final_web.spz`.
