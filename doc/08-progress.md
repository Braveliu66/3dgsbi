...
- `worker-fine` 权重检查已移除 AMB3R checkpoint；图片 fine SfM 使用 pycolmap，EDGS/RoMA 仍按需检查 `model-cache/roma/*`。
...
- GPU 端到端验收仍依赖可用 NVIDIA GPU、CUDA worker 镜像和 pycolmap/EDGS/RoMA 运行时；本地静态测试不能替代统一 worker CUDA 编译和短迭代真实图片验证。

## 下一步

- 在真实 GPU 环境启动统一 worker，确认 patched LM-RS rasterizer 的 `get_JTv/get_Diag/get_JTJv` 和 `MOBILEGS_COMPACT_BOX` smoke check 通过。
- 确认 `model-cache/roma/roma_outdoor.pth`、`model-cache/roma/roma_indoor.pth` 和 `model-cache/roma/dinov2_vitl14_pretrain.pth` 自动下载或手动预置完成后，用 3 张和 8 张以上 JPG/PNG 手机图像分别验证 pycolmap、EDGS/RoMA 初始化、DeblurMLP 训练、`final.ply`、`final_web.spz` 和 `metrics.json`；同时确认预览 LiteVGGT 任务仍可运行。
...
- The implementation keeps pycolmap as the SfM frontend and does not vendor the full EDGS repository, EDGS UI, Gradio app, original training scripts, or broad config tree.
- EDGS is enabled by default with `matches_per_ref=15000`, `nns_per_ref=3`, automatic `num_refs`, and `roma_model=outdoor`; `fine_edgs_enabled=false` keeps the pycolmap sparse path.
